...
export default {
  ...
  config: {
    ...
    configPath: {
      order: 6,
      title: 'PHPUnit Config Path',
      description: 'Set the PHPUnit config path if your project uses not standart configuration path.',
      type: 'string',
      default: 'phpunit.xml'
    },
    outputViewFontSize: {
    ...
    runnerSpecificOptions: {
      order: 8,
      title: 'PHPUnit specific options',
      description: 'PHPUnit specific options to run ',
      type: 'string',
      default: ''
    }
  },
  ...
  execute(filepath, functionName) {
    ...
    if (atom.config.get('atom-phpunit.configPath') !== 'phpunit.xml')
      cmd += ' --configuration=' + atom.config.get('atom-phpunit.configPath');

    if (atom.config.get('atom-phpunit.runnerSpecificOptions'))
      cmd += ' ' + atom.config.get('atom-phpunit.runnerSpecificOptions');
